...
sub _new_instance
{
    ...
    my (%args) = (
        ...
        'base_url' => 'http://api.dds.sertone.com/v1',
        @_
    );
...
elsif ($auth eq 'client_id') {
            
            my $api_key = $self->get_api_key_with_prefix('ClientID');
            if ($api_key) {
                $header_params->{'ClientID'} = $api_key;
            }
        }
        else {
